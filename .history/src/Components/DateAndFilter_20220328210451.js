...
function DateAndFilter(props){
  ...
  const saveRecord = (data) =>{
    ...
    for(let i =0; i<record.length;i++){
      if (!carerTemp.includes(record[i].carer)){
        carerTemp.push(record[i].carer);
      }
      if (!resTemp.includes(record[i].responder)){
        resTemp.push(record[i].responder);
      }
    }
    setCarer(true);
    setResident(true);
          ...
          >
            ...
            >
              <Grid item>
                ...
                <MultiSelectList label={"Carer"} names={carerTemp}/>
                {console.log(carerTemp)}
              </Grid>
              <Grid item><MultiSelectList label={"Resident"} names={resTemp}/></Grid>
              {console.log(resTemp)}
