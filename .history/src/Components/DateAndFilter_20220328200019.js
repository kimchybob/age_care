...
function DateAndFilter(props){
  ...
  const saveRecord = (data) =>{
      ...
      for(let i =0; i<record.length();i++){
        ...
        console.log("name:" + record[i].carer);
        if (!carer.includes(record[i].carer)){
          carer.push(record[i].carer);
        }
        if (!resident.includes(record[i].responder)){
          resident.push(record[i].responder);
        }
      }
      console.log("carer:" + carer);
